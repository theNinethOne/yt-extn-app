...
export default function AddSegmentModal( { startTime, endTime, videoId } ) {
  ...
  const segmentSubmitButtonHandler = async() => {
    ...
    const video = await pb.collection("videos").getFirstListItem(`videoId="${videoId}"`);
      ...
      <div>
        ...
        {isOpen && (
          <div className="fixed bg-black inset-0 bg-opacity-50 flex items-center justify-center z-40 flex-col">
            ...
            <div>
                <input type="text" placeholder="GIVE A NAME OF THIS SEGMENT" className="h-[100px] w-[250px] rounded-lg" onChange={(e) => setSegmentTitle(e.target.value)}/>
            </div>
            <div className="flex flex-row justify-center items-center p-1 m-1">
              ...
              <button onClick={ segmentSubmitButtonHandler} className="bg-red-500 p-2 rounded-lg m-2 w-[100px] h-[50px] text-white hover:scale-110 transition-all duration-300 ease-in-out">
                ADD
              </button>
            </div>
          </div>
